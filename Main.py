...
window = tk.Tk()
window.title("Covid 19 Forecasting Prediction")
...
message1 = tk.Label(window, text="Covid Prediction" ,bg="black"  ,fg="white"  ,width=45  ,height=1, font=('times', 30, 'bold')) 
...
def predict():
    ...
    cases_us = data[(data['Target'] == 'ConfirmedCases') & (data['Country_Region'] == ds)]
    deaths_us = data[(data['Target'] == 'Fatalities') & (data['Country_Region'] == ds)]
    print(cases_us.head())
    ...
    text.insert(END,"======Forecsted Confirmed Cases Count for Next Days======"+"\n\n")      
    text.insert(END,"Date        Confirmed Case Count"+"\n\n")      
    text.insert(END,"%s" %(predictions)+"\n\n")
   
        

list1 = ['US','India','China','Indonesia']    

message2 = tk.Label(window, text="Choose Country" ,bg="light blue"  ,fg="red"  ,width=25  ,height=1,font=('times', 15, ' bold ')) 
...
S1En.place(x=450, y=150)

trainImg = tk.Button(window, text="Forecast Covid19", command=predict  ,fg="red"  ,bg="light blue"  ,width=15  ,height=1, activebackground = "Red" ,font=('times', 15, ' bold '))
trainImg.place(x=650, y=250)




message2 = tk.Label(window, text="Forecasted Output" ,bg="white"  ,fg="black"  ,width=30  ,height=1,font=('times', 10, ' bold ')) 
message2.place(x=100, y=350)
